...
export function parseHighlight(card: MangaCard, rating: ContentRating): Highlight {
  // The score is the pill the app draws over the cover, which it draws on every shape of
  // tile — a title the site has graded nothing simply wears no pill.
  const badge = toBadge(formatScore(card.score));
...
export function parseChapters(data: ChaptersResponse, seriesId: string): Chapter[] {
  ...
  const chapters = numbers.map((value) => {
    ...
    return {
      ...
      webUrl: `${seriesUrl(seriesId)}/chapter-${value}-${TRANSLATION_TYPE}`,
      numbered,
    };
  });

  const highest = chapters.reduce((max, chapter) => Math.max(max, chapter.number), 0);
  const extras = chapters.filter((chapter) => !chapter.numbered);

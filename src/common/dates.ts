...
function parseKeyword(text: string): Date | undefined {
  ...
  if (/^(just now|now|moments? ago|a few seconds ago)$/.test(lower)) return new Date();
  if (/^today$/.test(lower)) return startOfToday();
  if (/^yesterday$/.test(lower)) return new Date(startOfToday().getTime() - UNIT_MS["day"]!);
  return undefined;
}
...
function parseTimestamp(text: string): Date | undefined {
  if (!/^\d{10}(\d{3})?$/.test(text)) return undefined;
...
export function parseChapterNumber(title: string | undefined, fallback: number): number {
  ...
  const match =
    /(?:chapter|chap|ch|episode|ep|#)\s*\.?\s*(\d+(?:\.\d+)?)/i.exec(title) ??
    /(\d+(?:\.\d+)?)/.exec(title);

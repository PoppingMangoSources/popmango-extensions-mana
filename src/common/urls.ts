 ...
 *
 * The JavaScriptCore environment these sources run in does not provide the
 * browser `URL`, so sources get this instead. Query items keep insertion
 * order, and a key set to an array is repeated once per value — which is how
 * most sites express multi-select filters.
 */
export class UrlBuilder {
  ...
  addPathComponent(component: string | number): this {
    ...
    if (value.length > 0) this.pathComponents.push(value);
    return this;
  }
  ...
  build(): string {
    ...
    if (this.pathComponents.length > 0) url += "/" + this.pathComponents.join("/");
    ...
    if (this.fragment !== undefined && this.fragment.length > 0) url += "#" + this.fragment;
...
export function resolveUrl(href: string, base: string): string {
  ...
  if (trimmed.startsWith("//")) {
    const scheme = /^([a-z][a-z0-9+.-]*):/i.exec(base)?.[1] ?? "https";

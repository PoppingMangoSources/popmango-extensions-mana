  ...
  },
  {
    ...
    match: new RegExp(
      String.raw`\b(tsundere|yandere|kuudere|dandere|deredere|shy|timid|cold|cool|calm|stoic|cheerful|happy|sad|gloomy|cynic|arrogant|proud|humble|kind|gentle|cruel|ruthless|violent|aggressive|passive|naive|innocent|clever|smart|intelligent|dumb|stupid|foolish|cunning|lazy|hardworking|diligent|studious|stubborn|loyal|devoted|selfish|selfless|jealous|possessive|protective|clumsy|serious|playful|mischievous|funny|silly|awkward|blunt|straightforward|honest|liar|manipulat|obsessive|insecure|confident|fearless|brave|coward|rude|polite|bossy|domineering|submissive|assertive|perceptive|capable|competent|incompetent|righteous|immoral|amoral|apathetic|emotionless|expressive|introvert|extrovert|optimist|pessimist|crybaby|crazy|eccentric|weird|mysterious|secretive|annoying|helpful|caring|nurturing|sadis|masochis|delinquent|rebellious|obedient|airhead|forgetful|absent-?minded|workaholic|glutton|perfectionist|hot-?blooded|short-?tempered|patient|impatient|charismatic|hot\b|anti-?social|friendly|unfriendly|flirt|pure|chaste|pervert|otaku|nerd|geek|tomboy|girly|childish|mature|responsible|carefree|reckless|cautious|greedy|generous|vengeful|forgiving|ambitious|determined|good|evil|villainous|heroic|strong|weak|powerful|helpless|lonely|broken|dense|oblivious)\b.*\b` +
        WHO +
        String.raw`\b|\b(tsundere|yandere|kuudere|dandere|deredere|tomboy|otaku|pervert|airhead|workaholic|perfectionist|coward|liar|glutton|crybaby|delinquent|masochist|sadist|introvert|extrovert|personality|temperament|character trait|attitude|emotion|feelings|gou-?dere|gap moe|gar\b|goths?\b|bad (boy|girl|habits)|chuunibyou|daydreamer|amoral|atypical|flirty|green flag|pacifist|nerds?\b|misfit|mary sue|non-?stereotypical|manipulation|penny pincher|shy\b|sly guy|pushover|pure-?hearted|troublemaker|serious|simple\b|sweet\b|obsessive|cheapskate|changed man|outsider|outlaw|capable woman|celeb\b|ojicon|overcoming shyness|red flag|boy next door|girl next door|chaebol|camboy|camgirl|troubled[ -]?past|naughty|scheming|averageguy|jock|hardboiled|misanthrop|bad luck|trust issues|pining|bodybuilder|gary stu|genki|hikikomori|haraguro|softoutsidestronginside|crude|unaccustomed)` +
        // The site carries a whole Indonesian vocabulary of lead descriptors written as one
        // word — `WanitaDingin`, `PriaTsundere`. They are all saying what someone is like.
        String.raw`|^(?:wanita|pria)`,
    ),
  },
  {
    id: "character-types",
    ...
    match: new RegExp(
      String.raw`\b` +
        WHO +
        String.raw`\b|\b(antagonist|villain|villainess|heroine|hero\b|anti-?hero|rival\b|sidekick|mentor|apprentice|disciple|underdog|prodigy|genius|chosen one|reincarnat|transmigrat|regressor|returnee|isekai|overpowered|weak to strong|multiple protagonists|ensemble|narrator|orphan|twin|triplet|quadruplet|sextuplet|only child|eldest|youngest|heir\b|successor|clone|doppelganger|alter ego|split personality|body swap|gender ?bend|possessed|amnesiac|immortal|mortal|star admirer|fanboys?|fangirls?|fans?\b|fandom|bachelor|mascots?\b)`,
    ),
  },
  {
    id: "relationships",
...
export type TypedTags = { included: string[]; excluded: string[] };

/**
 * Tag names typed into the search form, turned into the ids a search needs.
 *
 * A name is read the same way two spellings of one tag are folded together, so `Big Ass`,
 * `big ass` and `BigAss` all find the same tag and bring every id it was published under.
 * A name with `-` in front of it is asked to be excluded instead, and a name the site does
 * not carry is passed over rather than narrowing the search to nothing.
 */
export function resolveTypedTags(input: string, tagsById: Record<string, string>): TypedTags {
  const typed = input.trim();
  if (!typed) return { included: [], excluded: [] };

  const index = new Map<string, string[]>();
  for (const [id, name] of Object.entries(tagsById)) {
  ...
  for (const entry of typed.split(",")) {
    ...
    const ids = index.get(clusterKey(exclude ? name.slice(1) : name));
    if (ids) (exclude ? excluded : included).push(...ids);
  }
